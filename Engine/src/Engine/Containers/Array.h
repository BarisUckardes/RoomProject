...
namespace Engine
{
	...
	public:
		...
		unsigned int get_item_size_in_bytes() const { return sizeof(TValue); }
		...
		void clear();
		bool remove_index(unsigned int index);
	...
	*/
	...
	template<typename TValue>
	bool Array<TValue>::remove_index(unsigned int index)
	{
		if (index >= Cursor)
			return false;
	...
	template<typename TValue>
	bool Array<TValue>::remove(const TValue& element)
	{
		/*
		* Find it
		*/
